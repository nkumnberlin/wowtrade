...
export const fetchProfessionsByRecipeNames = (recipeNames: string[]) => prisma.craftingData.findMany({
	where: {
		item_name: {
			in: recipeNames
		}
...
export const getAllCraftedItems = () => prisma.craftingData.findMany() as PrismaPromise<Array<CraftingData>>;
...
export const updateCraftedItemsWithRecipeId = async (
  ...
  const writeOperationsByProfession = recipesByProfession.map((professionRecipes) =>
    professionRecipes.map((recipe) => prisma.craftingData.update({
		where: {
  ...
  return Promise.all(
	  writeOperationsByProfession
  );
};

export const removeAllCraftedItemsWithEmptyId = () => prisma.craftingData.deleteMany({
	where: {
